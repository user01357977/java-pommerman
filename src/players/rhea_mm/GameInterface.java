...
import players.rhea_mm.evo.Individual;
import players.rhea_mm.evo.Mutation;
import players.rhea_mm.hybrids.MCTSNode;
import players.rhea_mm.utils.FMBudget;
import players.rhea_mm.utils.RHEAParams_mm;
import players.rhea_mm.utils.Utilities;
...
public class GameInterface {
    ...
    GameInterface(RHEAParams_mm params, Random random, int playerID) {
        this.params = params;
        this.random = random;
...
//
    ...
    public double evaluate_offspring(Individual[][] copy_action_buffer, Mutation mutation, int offspring_idx, int agent_idx)
    {
        double last_value;
        GameState stateObsCopy;
        Types.ACTIONS[] allActions = new Types.ACTIONS[Types.NUM_PLAYERS];
        TIntHashSet[] genesToMutate = new TIntHashSet[Types.NUM_PLAYERS];
        for (int k=0; k < Types.NUM_PLAYERS; k++) {
            if (mutation != null) {
                mutation.findGenesToMutate();
                genesToMutate[k] = mutation.getGenesToMutate();
            }
        }

        stateObsCopy = rootState.copy();
        for (int i=0; i < params.individual_length; i++) {
            allActions[0] = action_mapping.get(copy_action_buffer[0][offspring_idx].get_action(i));
            for (int k = 1; k < Types.NUM_PLAYERS; k++) {
                if(agent_idx != 0) {
                    if (genesToMutate[k] != null && genesToMutate[k].contains(i)) {
                        mutation.mutateGeneToNewValue(copy_action_buffer[k][offspring_idx], i);
                    }
                } else {
                    if (genesToMutate[0] != null && genesToMutate[0].contains(i)) {
                        mutation.mutateGeneToNewValue(copy_action_buffer[0][offspring_idx], i);
                    }
                }
                allActions[k] = action_mapping.get(copy_action_buffer[k][offspring_idx].get_action(i));
            }
            stateObsCopy.next(allActions);
        }
        last_value = evaluateState(stateObsCopy);
        return last_value;
    }

    public double evaluate_state_opp(Individual[][] copy_action_buffer, Mutation mutation, int m, int n)
    {
        double last_value;
        GameState stateObsCopy;
        Types.ACTIONS[] allActions = new Types.ACTIONS[Types.NUM_PLAYERS];
//        TIntHashSet[] genesToMutate = new TIntHashSet[4];
//        for (int k=0; k < 4; k++) {
//            if (mutation != null) {
//                mutation.findGenesToMutate();
//                genesToMutate[k] = mutation.getGenesToMutate();
...
//        }
        ...
        stateObsCopy = rootState.copy();
        // loop through opponents' actions
        for (int i=0; i < params.individual_length; i++) {
            allActions[0] = action_mapping.get(copy_action_buffer[0][m].get_action(i));
            for (int k=1; k < Types.NUM_PLAYERS; k++) {
//                if (genesToMutate[k] != null && genesToMutate[k].contains(i)) {
//                    mutation.mutateGeneToNewValue(copy_action_buffer[k][n], i);
//                }
                allActions[k] = action_mapping.get(copy_action_buffer[k][n].get_action(i));
            }
            stateObsCopy.next(allActions);
        }
        last_value = evaluateState(stateObsCopy);
    ...
    public double evaluate_a_state(Individual[][] copy_action_buffer, Mutation mutation, int evo_idx, int agent_idx) {
        double last_value;
        GameState stateObsCopy;
        Types.ACTIONS[] allActions = new Types.ACTIONS[Types.NUM_PLAYERS];
            ...
            for (int k=0;k<Types.NUM_PLAYERS;k++) {
                if (k == agent_idx) {
                    if (genesToMutate != null && genesToMutate.contains(i)) {
                        mutation.mutateGeneToNewValue(copy_action_buffer[k][evo_idx], i);
                    }
                }
                allActions[k] = action_mapping.get(copy_action_buffer[k][evo_idx].get_action(i));
            }
            stateObsCopy.next(allActions);
        }
        last_value = evaluateState(stateObsCopy);
//        for (int k=0; k<4; k++) {
//            copy_action_buffer[k][evo_idx].set_value(last_value);
//        }
//        fmBudget.use();
        return last_value;

    }
    public double[][] evaluate_states(Individual[][] action_buffer) {//, double[][] Q) {
        double[][] last_values = new double[params.population_size][params.population_size];
        GameState stateObsCopy;
        Types.ACTIONS[] allActions = new Types.ACTIONS[Types.NUM_PLAYERS];
        ...
        for (int m=0; m<params.population_size; m++) {
            for (int n=0; n<params.population_size; n++) {
               ...
               for (int i=0; i<params.individual_length; i++) {
                   for (int k=0;k<Types.NUM_PLAYERS;k++) {
                       if (k==0) {
                           allActions[k] = action_mapping.get(action_buffer[k][m].get_action(i));
                       }else{
                           allActions[k] = action_mapping.get(action_buffer[k][n].get_action(i));
                       }
                   }
                   stateObsCopy.next(allActions);
               }
               last_values[m][n] = evaluateState(stateObsCopy);
            }
        }
//        fmBudget.use();
        return last_values;
...
//        individual.set_value(update_value);
        individual.set_value(state_value);
//        return update_value;
        return state_value;
    }
     ...
     */
    private int evaluateRollout(double[] values, GameState copy, int length, Individual individual, Mutation mutation) {
        // Keep track of where the rollout stopped (in case of early terminal state).
        int lastIdx = 0;

        // Retrieve the list of genes to mutate.
        TIntHashSet genesToMutate = null;
        TIntHashSet opp_genesToMutate = null;
        if (mutation != null) {
            genesToMutate = mutation.getGenesToMutate();
            opp_genesToMutate = mutation.opp_getGenesToMutate();
        }

        // separated mutations from the main function loop
...
//        return _stop_model();
     ...
     */
    public Types.ACTIONS translate(int action) {
