...
import players.rhea_minimax.evo.Individual;
import players.rhea_minimax.evo.Mutation;
import players.rhea_minimax.hybrids.MCTSNode;
import players.rhea_minimax.utils.FMBudget;
...
import static players.rhea_minimax.utils.Utilities.*;
...
public class GameInterface {
    ...
    GameInterface(RHEAParams_minimax params, Random random, int playerID) {
        this.params = params;
        this.random = random;
    ...
    public int[][] get_action_buffer(){
        int[][] action_buffer = this.rootState.actions_buffer;
    ...
    void initTick(GameState stateObs, ElapsedCpuTimer elapsedTimer) {
        ...
        switch (params.heurisic_type) {
            ...
            default:
            case CUSTOM_HEURISTIC: stateHeuristic = new CustomHeuristic(stateObs);
        }
        nActions = rootState.nActions();
    ...
    public double evaluate_opponent(Individual individual, Mutation mutation, int evaluation_update, Individual opponent) {
        ...
        int lastIdx = evaluateRollout_for_opponent(values, stateObsCopy, individual.get_length(), individual, mutation, opponent);
        ...
        opponent.set_value(update_value);
        return update_value;
    }
    public double evaluate_with_opponent(Individual individual, Mutation mutation, int evaluation_update, Individual opponent) {
        double[] values = new double[individual.get_length() + 1];
        GameState stateObsCopy = rootState.copy();
        if (params.evaluate_act == EVALUATE_ACT_LAST) {  // This doesn't need first state value
            values[0] = 0;
        ...
        // Evaluate subsequent states obtained by rolling through the actions
        int lastIdx = evaluateRollout_with_opponents(values, stateObsCopy, individual.get_length(), individual, mutation, opponent);
    ...
    private int evaluateRollout(double[] values, GameState copy, int length, Individual individual,
        ...
        Types.ACTIONS[] opponents_actions = new Types.ACTIONS[Types.NUM_PLAYERS-1];
    ...
    private int evaluateRollout_for_opponent(double[] values, GameState copy, int length, Individual individual,
                                               Mutation mutation, Individual opponents) {
        // Keep track of where the rollout stopped (in case of early terminal state).
        int lastIdx = 0;

        Types.ACTIONS[] opponents_actions = new Types.ACTIONS[Types.NUM_PLAYERS-1];
        // Retrieve the list of genes to mutate.
        TIntHashSet[] genesToMutate = new TIntHashSet[Types.NUM_PLAYERS-1];
        if (mutation != null) {
                ...
                if (individual != null) {
                    ...
                    for (int k=0; k<Types.NUM_PLAYERS-1; k++) {
                        if (genesToMutate[k] != null && genesToMutate[k].contains(i)) {
                            mutation.mutateGeneToNewValue(opponents, i+k*params.individual_length);
                        }
                    }
                    // Advance the state with the action in the individual
                    for (int k=0; k<Types.NUM_PLAYERS-1; k++) {
                        opponents_actions[k] = action_mapping.get(opponents.get_action(k * params.individual_length+i));
                    }
                    advanceState_with_opponents(copy, action_mapping.get(individual.get_action(i)), opponents_actions);
    ...
    private int evaluateRollout_with_opponents(double[] values, GameState copy, int length, Individual individual,
                                Mutation mutation, Individual opponents) {
        // Keep track of where the rollout stopped (in case of early terminal state).
        int lastIdx = 0;
    ...
    public void advanceState_with_opponents(GameState gs, Types.ACTIONS action, Types.ACTIONS[] opponents_moves) {
        int nPlayers = Types.NUM_PLAYERS;
        Types.ACTIONS[] actionsAll = new Types.ACTIONS[nPlayers];
...
//                actionsAll[i] = opponentModel(gs);
                actionsAll[i] = opponents_moves[k];
                k++;
            }
        }
